...
public class DownloadScheduler {
	...
	public void setTasks(List<DownloadInfo> tasks) {
		...
		this.filePath = tasks.get(0).getFilePath();
	}
	...
	public void start() {
		// 构造一个线程池
		threadPool = new ThreadPoolExecutor(1, 5, 1000,
		...
		printProgress();
		//全部任务完成后删除记录进度信息的文件
		for(File file : new File(this.filePath).listFiles(new FileFilter() {
			@Override
			public boolean accept(File pathname) {
				return pathname.getName().endsWith(".position");
			}
		})){
			file.delete();
		}
